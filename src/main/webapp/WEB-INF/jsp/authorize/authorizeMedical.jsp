<%-- 
    Document   : authorizeMedical
    Created on : Oct 10, 2016, 12:52:50 PM
    Author     : Shanaka
--%>
            ...
            <jsp:include page="/WEB-INF/jsp/common/header.jsp"></jsp:include>
                <div class="boxed">
                    <div id="content-container">
                        <div id="page-title">
                            <h1 class="page-header text-overflow">Authorize Pending Medical Reimbursement </h1>
                        </div>
                        <div class="panel">
                        <form:form id="authMedical" name="authMedical" class="form-horizontal" method="post">

                            <div class="panel-footer">
                                <div class="row">
                                    <div class="col-sm-9 col-sm-offset-3">
                                        <button type="submit" class="btn btn-info btn-labeled fa fa-save fa-2x focus"
                                                id="save" aria-pressed="false" value="auth">
                                            Authorize</button>

                                        <button type="submit" class="btn btn-danger btn-labeled fa fa-close fa-2x"
                                                id="edit" aria-pressed="false" value="reject">
                                            Reject</button>
                                    </div>
                                </div>
                            </div>
...
//                        $(".sub").is(':checked').checked = true;
                                ...
                                if ($(".scheckboxleave").is(':checked') == false) {
                                    ...
                                    this.checked = false;
                                                    ...
                                                    $(document).on("click", ":submit", function (e) {
                                                        var val = $(this).val();
                                                        $("#authMedical").submit(function (event) {
                                                            var action = "";
                                                            if (val === "auth") {
                                                                action = "${pageContext.request.contextPath}/authorize/authMedicalSave";
                                                                $("#authMedical").attr('action', action);
                                                            } else if (val === "reject") {
                                                                action = "${pageContext.request.contextPath}/authorize/rejectMedical";
                                                                $("#authMedical").attr('action', action);
                                                            } else {
                                                                alert("" + val);
        ...
        <script>
            ...
            $('#demo-dt-basic').DataTable().ajax.url(contextPath + "/authorize/pendingMedicalList").load();
