<%-- 
    Document   : authorizeFood
    Created on : Oct 10, 2016, 10:05:31 AM
    Author     : Shanaka
--%>
            ...
            <jsp:include page="/WEB-INF/jsp/common/header.jsp"></jsp:include>
                <div class="boxed">
                    <div id="content-container">
                        <div id="page-title">
                            <h1 class="page-header text-overflow">Authorize Pending Food Reimbursement </h1>
                        </div>
                        <div class="panel">
                        <form:form id="authFood" name="authFood" class="form-horizontal" method="post">
...
//                        $(".sub").is(':checked').checked = true;
                                ...
                                if ($(".scheckboxleave").is(':checked') == false) {
                                    ...
                                    this.checked = false;
                                                    ...
                                                    $(document).on("click", ":submit", function (e) {
                                                        var val = $(this).val();
                                                        $("#authFood").submit(function (event) {
                                                            var action = "";
                                                            if (val === "auth") {
                                                                action = "${pageContext.request.contextPath}/authorize/authFoodSave";
                                                                $("#authFood").attr('action', action);
                                                            } else if (val === "reject") {
                                                                action = "${pageContext.request.contextPath}/authorize/rejectFood";
                                                                $("#authFood").attr('action', action);
                                                            } else {
                                                                alert("" + val);
        ...
        <script>
            ...
            $('#demo-dt-basic').DataTable().ajax.url(contextPath + "/authorize/pendingFoodList").load();
