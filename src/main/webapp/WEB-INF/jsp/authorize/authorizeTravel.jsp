<%-- 
    Document   : authorizeTravel
    Created on : Oct 10, 2016, 11:48:26 AM
    Author     : Shanaka
--%>
            ...
            <jsp:include page="/WEB-INF/jsp/common/header.jsp"></jsp:include>
                <div class="boxed">
                    <div id="content-container">
                        <div id="page-title">
                            <h1 class="page-header text-overflow">Authorize Pending Travel Reimbursement </h1>
                        </div>
                        <div class="panel">
                        <form:form id="authTravel" name="authTravel" class="form-horizontal" method="post">
                                ...
                                <div class="panel-body">
                                    <table id="demo-dt-basic" class="table table-striped table-bordered" cellspacing="0"
                                        ...
                                        <thead>
                                            ...
                                            <tr>
                                                <th class="min-tablet"><input type="checkbox" class="scheckboxcb sub" name="" value="" onclick="checkSubItem()"></th>
                                                <th class="min-tablet">Project Name</th>
                                                <th class="min-tablet">Amount</th>                                  
                                                <th class="min-tablet">From</th>                                  
                                                <th class="min-tablet">To</th>                                  
                                                <th class="min-tablet">Distance</th>                                  
                                                <th class="min-tablet">Bill</th> 
...
//                        $(".sub").is(':checked').checked = true;

                            $(".sub").each(function () { //loop through each checkbox 

                                if ($(".scheckboxleave").is(':checked') == false) {
                                    this.checked = true;
                                } else {
                                    this.checked = false;
                                }
                                                    ...
                                                    $(document).on("click", ":submit", function (e) {
                                                        var val = $(this).val();
                                                        $("#authTravel").submit(function (event) {
                                                            var action = "";
                                                            if (val === "auth") {
                                                                action = "${pageContext.request.contextPath}/authorize/authTravelSave";
                                                                $("#authTravel").attr('action', action);
                                                            } else if (val === "reject") {
                                                                action = "${pageContext.request.contextPath}/authorize/rejectTravel";
                                                                $("#authTravel").attr('action', action);
                                                            } else {
                                                                alert("" + val);
        ...
        <script>
            ...
            $('#demo-dt-basic').DataTable().ajax.url(contextPath + "/authorize/pendingTravelList").load();
