<%-- 
    Document   : addUserStatus
    Created on : Jun 13, 2016, 4:02:02 PM
    Author     : Shanaka
--%>
...
<html lang="en">
    <head>
        ...
        <script type="application/javascript">
            ...
            function closePage() {
            document.userStatus.action = "${pageContext.request.contextPath}/user/userStatus";
            document.userStatus.submit();
            }
        </script>
                    ...
                    <div id="content-container">
                        ...
                        <c:choose>
                            <c:when test="${recordType=='i'}">
                                <h1 class="page-header text-overflow">Create User Status</h1>
                            </c:when>
                            <c:when test="${recordType=='e'}">
                                <h1 class="page-header text-overflow">Edit User Status</h1>
                            </c:when>
                        </c:choose>
                            
                        </div>
                        <div class="panel">
                        <form:form id="userStatus" name="userStatus" class="form-horizontal" method="post">

                            
                                <div class="panel-body">
                                      
                                    <div class="form-group">
                                        <label for="type_name" class="col-sm-3 control-label">Status Name<span
                                                class="text-danger">*</span></label>
                                        <div class="col-sm-6">
                                            <input type="text" placeholder="Status Name" class="form-control input-lg"
                                                   id="emp_status" name="emp_status" value="">
                                        </div>
                                    </div> 
    ...
    <script>
                                                ...
                                                $(document).on("click", ":submit", function (e) {
                                                    var val = $(this).val();
                                                    $("#userStatus").submit(function (event) {
                                                        var action = "";
                                                        if (val === "Save") {
                                                            action = "${pageContext.request.contextPath}/userStatus/saveUserStatus";
                                                            $("#userStatus").attr('action', action);
                                                        } else if (val === "Edit") {
                                                            action = "${pageContext.request.contextPath}/userStatus/editUserStatus";
                                                            $("#userStatus").attr('action', action);
                                                        }  else {
                                                            alert("" + val);
        ...
        <c:if test="${errorMessage!=null && errorMessage!=''}">
            ...
            <c:forEach var="UserStatusDTO" items="${userStat}">
            <script type="text/javascript">            
                    $('#emp_status').val('${UserStatusDTO.emp_status}');
                    $('#status_id').val(${UserStatusDTO.status_id});
