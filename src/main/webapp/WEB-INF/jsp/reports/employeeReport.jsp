<%-- 
    ...
    Created on : Oct 16, 2016, 4:04:38 PM
    Author     : Shanaka
--%>
    ...
    <body>
            ...
            <jsp:include page="/WEB-INF/jsp/common/header.jsp"></jsp:include>
                <div class="boxed">
                    <div id="content-container">
                        <div id="page-title">
                            <h1 class="page-header text-overflow">Employee Report Generate</h1>                           
                        </div>
                        <div class="panel">
                        <form:form id="empRep" name="empRep" class="form-horizontal" method="post">
                                    ...
                                    <div class="col-sm-6">
                                        <select name="user_role_id" id="user_role_id" class="form-control">
                                             <c:forEach var="item" items="${userRoleList}">
                                                <c:if test="${item.user_role_id != 'Admin'}">
                                                    <option value="${item.user_role_id}" ${item.user_role_id == emp.user_role_id ? 'selected="selected"' : ''}>${item.role_name}</option>
                                                </c:if> 
                                            </c:forEach>
        ...
        <script>
                                            ...
                                            $(document).on("click", ":submit", function (e) {
                                                var val = $(this).val();
                                                $("#empRep").submit(function (event) {
                                                    var action = "";
                                                    if (val === "view") {
                                                        action = "${pageContext.request.contextPath}/report/employeePdf";
                                                        $("#empRep").attr('action', action);
                                                    } else if (val === "download") {
                                                        action = "${pageContext.request.contextPath}/report/downEmployeePdf";
                                                        $("#empRep").attr('action', action);
                                                    } else {
                                                        alert("" + val);
